...
class _CoviScreenState extends State<CoviScreen> {
  ...
  Future<void> _bootstrap() async {
    ...
    final me = HiveService.getUserById(_userId);
    if (me != null) {
      _userRole = me['role'] as String? ?? 'buyer';
    }
    _loadImages();
  }
  ...
  Future<void> _loadImages() async {
    ...
    try {
      final result = HiveService.getCoviImagesEnriched(viewerUserId: _userId);
  ...
  Future<void> _pickAndAddImage() async {
    try {
      ...
      await HiveService.addCoviImage(
        imagePath: file.path,
        caption: captionController.text,
        story: storyController.text,
        uploaderId: _userId,
        uploaderRole: _userRole,
      );
  ...
  Future<void> _deleteImage(int id) async {
    try {
      await HiveService.deleteCoviImage(id);
  ...
  Future<void> _toggleLike(int coviId, bool isLiked) async {
    try {
      await HiveService.toggleCoviLike(coviId: coviId, userId: _userId);
      _loadImages();
    } catch (e) {
          ...
          : _images.isEmpty
              ...
              : GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final likeCount = (img['likes'] is int)
                        ? img['likes'] as int
                        : ((img['likes'] is num) ? (img['likes'] as num).toInt() : 0);
                    final liked = (img['liked'] is bool)
                        ? (img['liked'] as bool)
                        : ((img['liked'] is int) ? (img['liked'] as int) > 0 : false);
                    return Card(
                      elevation: 3,
