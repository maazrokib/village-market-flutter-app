...
class _AdminMessagesState extends State<AdminMessages> {
  ...
  Future<void> _loadMessages() async {
    ...
    try {
      ...
      final messages = HiveService.getMessagesForReceiver(adminId, filter: _selectedFilter);
  ...
  Future<void> _sendReply(int receiverId, String subject, String message) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final adminId = prefs.getInt('user_id') ?? 1;
      await HiveService.addMessage(subject: subject, message: message, senderId: adminId, receiverId: receiverId);
                ...
                DropdownButtonFormField<String>(
                  ...
                  onChanged: (value) async {
                    ...
                    if (value == 'specific') {
                      ...
                      try {
                        _users = HiveService.getAllNonAdminUsers();
  ...
  Future<void> _sendMessageToUsers(String role, int? userId, String subject, String message) async {
    try {
      ...
      if (role == 'specific' && userId != null) {
        ...
        if (u != null) targetUsers = [u];
      } else if (role == 'all') {
        targetUsers = HiveService.getAllNonAdminUsers();
      } else if (role == 'farmer') {
        targetUsers = HiveService.getUsersByRole('farmer');
      } else if (role == 'buyer') {
        targetUsers = HiveService.getUsersByRole('buyer');
      }
