...
class _AdminHomeState extends State<AdminHome> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      // Load from Hive
      _users = HiveService.getAllUsers()..sort((a, b) => (a['name'] as String).compareTo(b['name'] as String));
      final products = HiveService.getAllProducts();
      final usersById = { for (final u in _users) u['id']: u };
      _products = products.map((p) => {
        ...p,
        'farmer_name': usersById[p['farmer_id']]?['name'],
      }).toList()
        ..sort((a, b) => (b['created_at'] as String).compareTo(a['created_at'] as String));

      final orders = HiveService.getAllOrdersEnriched();
      _orders = orders.map((o) {
        final p = products.firstWhere((pp) => pp['id'] == o['product_id'], orElse: () => {});
        final buyer = usersById[o['buyer_id']];
        final farmer = usersById[p['farmer_id']];
        return {
          ...o,
          'product_name': p['name'],
          'image': p['image'],
